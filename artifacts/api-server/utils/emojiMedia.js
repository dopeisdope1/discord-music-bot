...
/** Le binaire ffmpeg embarqué, ou la commande système disponible sur Replit. */
function binaireFfmpeg() {
  if (process.env.FFMPEG_PATH) return process.env.FFMPEG_PATH;

  try {
    const chemin = require("ffmpeg-static");
    if (chemin && fs.existsSync(chemin)) return chemin;
  } catch {
    // Le paquet peut ne pas embarquer son binaire sur certaines plateformes.
  }
