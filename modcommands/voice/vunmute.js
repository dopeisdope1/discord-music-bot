const { PermissionFlagsBits } = require("discord.js");
const { LEVEL } = require("../../utils/permLevels");
const { UsageError, BotError } = require("../../utils/modErrors");
const { resolveVoiceMember, assertBotCan, assertCanActOn } = require("../../utils/voiceModHelpers");
const { sendLog } = require("../../utils/actionLogger");

module.exports = {
  name: "vunmute",
  aliases: ["vdemute"],
  category: "voice",
  description: "Rend la parole à un membre en vocal",
  usage: "&vunmute <@mention | id>",
  level: LEVEL.CONFIGURABLE,
  async execute(ctx) {
    if (!ctx.args[0]) throw new UsageError(this.usage);
    assertBotCan(ctx, PermissionFlagsBits.MuteMembers, "Rendre muet les membres");

    const target = await resolveVoiceMember(ctx, ctx.args[0], this.usage);
    assertCanActOn(ctx, target);
    if (!target.voice.serverMute) throw new BotError(`**${target.user.tag}** n'est pas muet.`);

    await target.voice.setMute(false, `Unmute vocal par ${ctx.author.tag}`);

    sendLog(ctx.client, ctx.guildId, "voice", {
      title: "Unmute vocal",
      description: `${target.user.tag} a retrouvé la parole en vocal.`,
      actor: ctx.author,
    });

    await ctx.reply(ctx.card({ title: `${target.user.tag} a retrouvé la parole.` }));
  },
};
