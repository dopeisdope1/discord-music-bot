...
// Discord limite à 5 menus contextuels "utilisateur" par application, d'où
// les bascules (muet/parole et sourd/écoute sur une seule entrée chacune)
// plutôt qu'une entrée par sens.
//
// `permission` fait le lien avec les commandes texte : c'est le même nom que
// dans &panel > Permissions, donc un slot qui donne `vmute` donne aussi le
// clic droit "Muet vocal". Rien de nouveau à configurer.
const MENUS = {
  "Déplacer en vocal": { action: "move", permission: () => "vmove" },
  "Muet vocal": { action: "mute", permission: (target) => (target.voice.serverMute ? "vunmute" : "vmute") },
  "Sourd vocal": { action: "deaf", permission: (target) => (target.voice.serverDeaf ? "vundeaf" : "vdeaf") },
  "Déconnecter du vocal": { action: "disconnect", permission: () => "vkick" },
...
// Mêmes garde-fous de hiérarchie que la modération texte.
...
async function handleContextMenu(interaction) {
  ...
  const deny = denyReason(menu.permission(target), interaction.member);
